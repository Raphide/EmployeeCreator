...
public class EmployeeEndToEndTest {
    ...
    public void setup() {
        ...
        employee1.setIsFullTime(true);
        employee1.setIsPermanent(false);
    ...
    public void getAllEmployees() {
        given().when().get("/employees").then().statusCode(HttpStatus.OK.value()).body("$", hasSize(2))
                ...
                .body("lastName", hasItems("Mason", "Tentacles")).body("employeeUser", hasItems("chemas", "squten")).body("employeeEmail", hasItems("chemas@company.com", "squten@company.com"));
    }

    @Test
    public void getEmployeeById() {
        given().when().get("employees/{id}", employeeId).then().statusCode(HttpStatus.OK.value()).body("firstName", equalTo("Cheryl")).body("suburb", equalTo("Silent Hill"));
    }
    ...
    public void createEmployee_success(){
        ...
        given().contentType(ContentType.JSON).body(data).when().post("/employees").then().statusCode(HttpStatus.CREATED.value()).body("firstName", equalTo("Greg")).body("gender", equalTo("nonbinary")).body("street", equalTo("57 Greg lane")).body("id", notNullValue());

        given().when().get("/employees").then().statusCode(HttpStatus.OK.value()).body("$", hasSize(3)).body("middleName", hasItems("Heather", null, "Greggor")).body("isFullTime", hasItems(true, true, false));
    }
